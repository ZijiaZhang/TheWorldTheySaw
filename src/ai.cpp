...
void AISystem::step(float elapsed_ms)
{
	...
	for (auto& e : ECS::registry<Enemy>.entities) {
		enemy_ai_step(e, elapsed_ms, 200.f);
	}
}

void AISystem::enemy_ai_step(ECS::Entity e, float elapsed_ms, float radius) {
	for (auto& enemy : ECS::registry<Enemy>.entities) {
		...
		if (!ECS::registry<Soldier>.components.empty()) {
			...
			if (soldier.has<Motion>()) {
				...
				auto path = find_path_to_location(enemy, soldier_motion.position, radius);
