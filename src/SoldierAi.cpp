...
void SoldierAISystem::makeDecision(ECS::Entity& soldier_entity, float elapsed_ms)
{
 ...
 if (SoldierAISystem::isEnemyExists())
 {
  ...
  Soldier::state aState = soldier.soldierState;
  if (SoldierAISystem::isEnemyExistsInRange(soldier_motion, enemyMotion, 300) && aState == Soldier::WALK_FORWARD_AND_SHOOT) {
   soldier.soldierState = Soldier::WALK_BACKWARD_AND_SHOOT;
   SoldierAISystem::walkBackwardAndShoot(soldier_motion, enemyMotion);

  }
  else if (SoldierAISystem::isEnemyExistsInRange(soldier_motion, enemyMotion, 400) && aState == Soldier::WALK_BACKWARD_AND_SHOOT) {
   soldier.soldierState = Soldier::WALK_BACKWARD_AND_SHOOT;
   SoldierAISystem::walkBackwardAndShoot(soldier_motion, enemyMotion);
  }
  else
  {
   soldier.soldierState = Soldier::WALK_FORWARD_AND_SHOOT;
   SoldierAISystem::walkForwardAndShoot(soldier_motion, enemyMotion);
  }
 }
 else
 {
  soldier.soldierState = Soldier::IDLE;
